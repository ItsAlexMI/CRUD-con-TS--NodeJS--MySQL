...
app.get('/student', (req: Request<any, any, any, any> & { session?: CustomSessionData }, res: Response) => {
...
app.post('/group', (req: Request, res: Response) => {
  ...
  db.get(`SELECT id FROM usuarios WHERE username = ? AND role = 'Profesor'`, [username], (error, row: UsuarioRow) => {
    if (error) {
...
app.post('/join-group', (req: Request<any, any, any, any> & { session?: CustomSessionData }, res: Response) => {
  ...
  db.get<GrupoRow>(`SELECT id, estudiantes_ids FROM grupos WHERE codigo = ?`, [groupname], (error, row) => {
    if (error) {
